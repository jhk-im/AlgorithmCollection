...
fun main(args: Array<String>) {
    print(Solution().solution("aukks", "wbqd", 5))
}
...
class Solution {
    fun solution(s: String, skip: String, index: Int): String {
        ...
        var mapCount = 0
        val skipMap = skip.map { it to mapCount++ }.toMap()

        var alphabetCount = 0
        for (i in 'a' .. 'z'){
            if (!skipMap.containsKey(i)) {
                alphabetMap[i] = alphabetCount++
            }
        }

        val keyMap = alphabetMap.entries.associate { it.value to it.key }
        val length = keyMap.size - 1
        for (value in s) {
            alphabetMap[value]?.let { idx ->
                val next = idx + index
                answer += if (next > length) {
                    keyMap[next % (length + 1)].toString()
                } else {
                    keyMap[next].toString()
                }
            }
        }
