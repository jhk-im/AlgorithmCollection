...
fun main(args: Array<String>) {
    val keymap = arrayOf("ABACD", "BCEFD") // 자판
    val targets = arrayOf("ABCD", "AABB") // 입력문자
    print(Solution().solution(keymap, targets).contentToString())
}

class Solution {
    fun solution(keymap: Array<String>, targets: Array<String>): IntArray {
        val answer = IntArray(targets.size) { _ -> 0 }

        val map = mutableMapOf<Char, Int>()
        keymap.forEach { key ->
            for ((index, value) in key.withIndex()) {
                if (map.containsKey(value)) {
                    map[value]?.let { prevIndex ->
                        if (prevIndex > index) {
                            map[value] = index
                        }
                    }
